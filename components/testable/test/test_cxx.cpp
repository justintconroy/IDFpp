...
/* Note: When first exception (in system) is thrown this test produces memory
   leaks report (~300 bytes):
   - 8 bytes are allocated by __cxa_get_globals() to keep __cxa_eh_globals
   - 16 bytes are allocated by pthread_setspecific() which is called by
   __cxa_get_globals() to init TLS var for __cxa_eh_globals
...
TEST_CASE("c++ exceptions emergency pool",
          "[cxx] [exceptions] [ignore] [leaks=" LEAKS "]")
{
  void **p, **pprev = NULL;
  int thrown_value = 0;
...
TEST_CASE("frequent interrupts don't interfere with c++ exceptions",
          "[cxx] [exceptions] [leaks=816]")
{ // if exception workaround is disabled, this is almost guaranteed to fail
  const esp_timer_create_args_t timer_args{
      timer_cb, NULL, ESP_TIMER_TASK, "crash_timer"};
...
// TEST_CASE("std::out_of_range exception when -fno-exceptions",
// "[cxx][reset=abort,SW_CPU_RESET][fails]")
// {
//     std::vector<int> v(10);
//     v.at(20) = 42;
//     TEST_FAIL_MESSAGE("Unreachable because we are aborted on the line
//     above");
// }

// TEST_CASE("std::bad_alloc exception when -fno-exceptions",
// "[cxx][reset=abort,SW_CPU_RESET][fails]")
// {
...
template <typename T>
__attribute__((unused)) static void test_binary_operators()
{
  T flag1 = (T)0;
  T flag2 = (T)0;
  flag1   = ~flag1;
...
// Add more types here. If any flags cannot pass the build, use FLAG_ATTR in
// esp_attr.h
